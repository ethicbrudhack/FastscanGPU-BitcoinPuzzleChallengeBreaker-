...
def run_worker(args):
    ...
    # [PL] v2.0: token idzie takze do /config. Nowy serwer wydaje zakres
    #      (shifted_lo/hi) tylko zalogowanym - anonim go nie dostanie, bo z
    #      zakresu dalo sie odzyskac sekret puli. Bez tokenu tez zadziala:
    #      zakres przyjdzie z /work (patrz seg_lo/seg_hi nizej).
    # [EN] v2.0: the token is sent to /config as well. The new server serves the
    #      range (shifted_lo/hi) to logged-in miners only, since the range leaked
    #      the pool secret. Works without a token too - the range then comes from
    #      /work (see seg_lo/seg_hi below).
    _cfg_url = f"{base}/config"
    if worker_token:
        _cfg_url += f"?worker={worker}&token={worker_token}"
    cfg = http_get(_cfg_url)
    ...
    while True:
        ...
        if time.time() - last_config_ts > CONFIG_REFRESH:
            try:
                # [PL] v2.0: token takze przy odswiezaniu (jak wyzej).
                # [EN] v2.0: token on refresh as well (same as above).
                cfg = http_get(_cfg_url)
                sb, eb = cfg["start_bit"], cfg["end_bit"]
                sx, sy = cfg["offset_sx"], cfg["offset_sy"]
                mode = cfg.get("mode", "puzzle")
                scan_target = cfg.get("scan_target") or cfg.get("puzzle_addr")
                # [PL] Nie nadpisuj dzialajacego zakresu wartoscia None, jesli
                #      serwer go nie przyslal (np. brak tokenu) - zakres per
                #      segment z /work i tak ma pierwszenstwo.
                # [EN] Don't overwrite a working range with None when the server
                #      omits it (e.g. no token) - the per-segment range from
                #      /work takes precedence anyway.
                shifted_lo = cfg.get("shifted_lo") or shifted_lo
                shifted_hi = cfg.get("shifted_hi") or shifted_hi
                _cached_config.update(cfg)
                last_config_ts = time.time()
        ...
        # ============================================================
        # [PL] v2.0 - zakres pobierany PER SEGMENT z /work (fallback na /config).
        #      Nowy serwer podaje shifted_lo/hi razem z segmentem, co pozwolilo
        #      usunac te dane z globalnego /config (poprawka bezpieczenstwa).
        #      Fallback zapewnia, ze ten worker dziala TAKZE ze starym serwerem -
        #      mozna aktualizowac w dowolnej kolejnosci, nic nie przestanie
        #      dzialac.
        # [EN] v2.0 - the range is taken PER SEGMENT from /work (with a /config
        #      fallback). The new server sends shifted_lo/hi along with the
        #      segment, which allowed removing them from the global /config
        #      (security fix). The fallback keeps this worker compatible with the
        #      old server too, so updates can happen in any order.
        # ============================================================
        seg_lo = seg.get("shifted_lo") or shifted_lo
        seg_hi = seg.get("shifted_hi") or shifted_hi
        if not seg_lo or not seg_hi:
            print("[PL] Serwer nie podal zakresu (shifted_lo/hi) - ponawiam za 15s.")
            print("[EN] Server provided no range (shifted_lo/hi) - retrying in 15s.")
            time.sleep(15)
            continue
        ...
        if gpu_proc is None:
            ...
            cmd = [
                args.binary, arg1,
                "--range-lo", seg_lo, "--range-hi", seg_hi,
                f"--mode={scan_mode}",
            ...
            cmd_display = [
                args.binary, arg1,
                "--range-lo", seg_lo[:16]+"...", "--range-hi", seg_hi[:16]+"...",
                f"--mode={scan_mode}",
        ...
        while not segment_done:
            ...
            if "[POOL-PERSISTENT]" in line and ("przetworzone" in line or "processed" in line):
                # Czekaj az binarka zacznie czekac na pool_next.txt
                time.sleep(0.5)
                segment_done = True
            elif "[POOL]" in line and ("Koniec zadania" in line or "Task complete" in line):
