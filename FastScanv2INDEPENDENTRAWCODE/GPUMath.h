...
// ModSub256isOdd: liczy TYLKO parytet (bit LSB) wyniku (a-b) mod P, bez pelnego
// wyniku. Uzywane gdy potrzebujemy tylko wiedziec czy Y jest parzyste/nieparzyste
// (prefiks 0x02/0x03 compressed pubkey) - o wiele tansze niz pelny _ModSub256.
// Dodanie P (P[0] jest NIEPARZYSTE) odwraca bit 0 wyniku gdy byl borrow.
// Patrz CUDACyclone CUDAMath.h ModSub256isOdd.
__device__ __forceinline__ uint8_t _ModSub256isOdd(uint64_t *a, uint64_t *b)
{
    uint64_t r0, r1, r2, r3, t;
    USUBO(r0, a[0], b[0]);
    USUBC(r1, a[1], b[1]);
    USUBC(r2, a[2], b[2]);
    USUBC(r3, a[3], b[3]);
    USUB(t, 0ULL, 0ULL);
    uint64_t T0 = 0xFFFFFFFEFFFFFC2FULL & t;
    UADDO1(r0, T0);
    (void)r1; (void)r2; (void)r3; // carry chain only, values unused
    return (uint8_t)(r0 & 1ULL);
}
